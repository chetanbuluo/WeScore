...
class CCMiniModel {
	...
	static async insertBatch(data = [], size = 1000) {
		...
		if (this.ADD_ID) {
			let idField = this.CCMINI_FIELD_PREFIX + 'ID';
			for (let k in data)
				if (!ccminiUtil.isDefined(data[k][idField])) data[k][idField] = CCMiniModel.makeID();
		}
		...
		if (this.UPDATE_TIME) {
			...
			for (let k in data) {
				if (!ccminiUtil.isDefined(data[k][addField])) data[k][addField] = timestamp;
				if (!ccminiUtil.isDefined(data[k][editField])) data[k][editField] = timestamp;
			}
		...
		if (this.UPDATE_IP) {
			...
			for (let k in data) {
				if (!ccminiUtil.isDefined(data[k][addField])) data[k][addField] = ip;
				if (!ccminiUtil.isDefined(data[k][editField])) data[k][editField] = ip;
			}
		...
		return await ccminiDbUtil.insertBatch(this.CL, data, size);
	}
