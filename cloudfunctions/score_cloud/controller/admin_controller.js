...
class AdminController extends CCMiniController {
	...
	async getGradeList() {
		...
		let rules = {
			search: 'string|min:1|max:30|name=搜索条件',
			id: 'string|min:1|max:100|name=newsId',
			sortType: 'string|name=搜索类型',
			sortVal: 'name=搜索类型值',
			orderBy: 'object|name=排序',
			whereEx: 'object|name=附加查询条件',
		...
		let service = new AdminService();
		let result = await service.getGradeList(input);

		// 数据格式化
		let list = result.list;
		for (let k in list) {

			list[k].GRADE_ADD_TIME = ccminiTimeUtil.timestamp2Time(list[k].NEWS_ADD_TIME);
	...
	async insertNews() {
		// 数据校验
		let rules = {
			title: 'required|string|min:5|max:50|name=科目标题',
			cate: 'required|string|min:1|max:15|name=科目分类',
	...
	async delGrade() {
		...
		let service = new AdminService();
		let result = service.delGrade(input.id);
	...
	async clearGrade() {
		...
		let service = new AdminService();
		let result = service.clearGrade(input.id);
	...
	async importGrade() {
		...
		// 数据校验
		let rules = {
			cloudId: 'required|string|min:5|max:500|name=cloudId',
			id: 'required|string|min:5|max:500|name=id',
		};
		...
		let service = new AdminService();
		return await service.importGrade(input.id, input.cloudId);
